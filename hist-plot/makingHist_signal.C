...
int main(int argc,char **argv){
    ...
    TH1D *Cutflow_w = new TH1D("cutflow_w", "cutflow_w", 9, 0.5, 9.5);
	...
	ifstream infile;
	string root_list="";
	infile.open("./Input/Hprime.txt");
	string p = "mc16"+period;
	while(getline(infile,root_list)){
		if((root_list.find(mass) != string::npos) && (root_list.find(p) != string::npos)){
	...
	//        
				...
				}else cout << "No cutflow_weighted in " << endl;
		}
			...
			if ((t != 0)){
				...
				for (int i = 0; i < nEntries; i++){
					...
					Cutflow->Fill(9);
					//TH1D *Cutflow_w = (TH1D*)Cutflow->Clone();
					//Cutflow_w->Scale(w);
					Cutflow_w->Add(Cutflow,w);
		...
		TFile *fout = TFile::Open("Hprime_"+mass+"_"+period+".root", "recreate");
